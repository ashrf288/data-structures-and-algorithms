...
const screenForNames = (arr) => {
  // Solution code here...
 let regx1=/^(Mr||Dr||Mrs||Ms).\s[A-Z]/
  let newArr=arr.filter(ele=>{
     return  regx1.test(ele) &&ele
  })
  return newArr
}
...
const toTitleCase = (arr) => {
  ...
  let newArr= arr.map(ele => {
    return ele.replace(regx,ele[0].toUpperCase());;
  })
  return newArr
};
...
const isSecure = (url) => {
  // Solution code here...
  let regx=/^(https:\/\/)/
  return (regx.test(url) ? true : false);
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  let arrays=board.forEach(arr=>{
    if(arr[0]===arr[1]&&arr[1]===arr[2]&&arr[0]===arr[2]&& arr[0]===''){return true}
  })
  return arrays[0]
};
