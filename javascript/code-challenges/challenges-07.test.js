...
const sortStarWarsCharacters = (starWarsArr) => {
  ...
  let newArr=starWarsArr.sort(compare,0)
  return newArr
}
...
const removeThree = (idx, arr) => {
 ...
 let newArr=arr.filter((item,index)=>{
   return !(idx===index||idx+1===index||idx+2===index)&&item
 })
  return newArr
};
...
let newArr=  arr.map(ele=>{
 let index= ele.indexOf(' ')
    let ingr=ele.slice(index)
    let trimmed=ingr.trim()
    console.log(trimmed)
    let secoundIndx=trimmed.indexOf(' ')
    console.log(secoundIndx)
    let ingred=trimmed.slice(secoundIndx)
    // console.log(ingred)

    let removed= ingred.trim()
    return removed
  })
result=newArr
