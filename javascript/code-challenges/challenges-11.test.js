...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let arr = input.map((array) => {
    let a = [];
    return (a = array.filter((ele) => {
      return (ele % 5 === 0 && typeof(ele)==='number') && ele;
    }));
  });

  let array = arr.map((array) => {
    return  array.length!==0? array.map((ele) => Math.pow(2, ele)):[]
  });
  return array;
};
...
let findShortest = (data) => {
  ...
  let name = data[0] 
    data.filter(ele =>
       {  if (parseInt(ele.height) < parseInt(name.height))    
           name = ele}) 
             return name.name;
};
