...
const maxInArray = (arr) => {
  // Solution code here...
  let num=(a,b)=>Math.max(a,b);
   let maxNumber= arr.reduce(num);
   return maxNumber;
};
...
const checkValues = (obj, value) => {
  ...
  let theItem=arr.map(ele=>{
    return value===ele?true:false
  })
  return theItem[0]
...
const hasChildrenValues = (arr, character) => {
 ...
 for (let j = 0; j < arr.length; j++) {
    let array1 = Object.values(arr[j]);
    let array2 = array1[2];
    if (array2) {
      if (typeof (array2) == 'object') {
        for (let i = 0; i < array2.length; i++) {
          if (array2[i] === character) { value = true; break; }

        }
       if(value){ break;}
      }
      else { if (array2[j] === character) { value = true; break; }else{ value = false;} }
